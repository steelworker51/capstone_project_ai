...
def show_transaction_type_percentages(transactions):
    """Display percentage of total amount by transaction type."""
    if not transactions:
    ...
    total_amount = totals['debit'] + totals['credit']

    if total_amount == 0:
        print("Total amount is zero; cannot calculate percentages.")
        return

    debit_percent = (totals['debit'] / total_amount) * 100
    credit_percent = (totals['credit'] / total_amount) * 100

    print("\n--- Transaction Amount Percentages by Type ---")
    print(f"Total Amount: ${total_amount:.2f}")
    print(f"Debit: {debit_percent:.2f}% (${totals['debit']:.2f})")
    print(f"Credit: {credit_percent:.2f}% (${totals['credit']:.2f})\n")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print("3. Show Percentage by Transaction Type")
        print("4. Exit")
        ...
        if choice == '1':
            ...
            show_transaction_type_percentages(transactions)
        elif choice == '4':
            print("Goodbye!")
            break
        else:
...
# lines 70-95 define the function action_type_percentages 
# line 104 updates the menu to give you an option to show percentage by transaction type
