import csv
from datetime import datetime

def load_transactions(filename='financial_transactions.csv'):
    transactions = []
    skipped_rows = []  # to store info about skipped rows

    try:
        with open(filename, mode='r', newline='') as file:
            reader = csv.DictReader(file)
            for row_num, row in enumerate(reader, start=2):  # start=2 to account for header line
                try:
                    # Parse date
                    date = datetime.strptime(row['date'], '%Y-%m-%d')

                    # Parse amount
                    try:
                        amount = float(row['amount'])
                    except ValueError:
                        skipped_rows.append((row_num, f"Invalid amount: {row['amount']}"))
                        continue

                    # Adjust amount if it's a debit
                    if row['type'].lower() == 'debit':
                        amount = -amount

                    # Build and store transaction
                    transaction = {
                        'date': date,
                        'description': row['description'],
                        'amount': amount,
                        'type': row['type'].lower()
                    }
                    transactions.append(transaction)

                except ValueError as ve:
                    skipped_rows.append((row_num, f"Value error: {ve}"))

    except FileNotFoundError:
        print(f"Error: The file '{filename}' was not found.")

    return transactions, skipped_rows

# === Main execution block ===
if __name__ == '__main__':
    print("Loading transactions...")
    transactions, skipped_rows = load_transactions()

    print(f"\nLoaded {len(transactions)} valid transactions.")

    # Print skipped row info
    if skipped_rows:
        print("\nSkipped rows due to errors:")
        for row_num, reason in skipped_rows:
            print(f" - Row {row_num}: {reason}")
    else:
        print("\nNo rows were skipped.")

# === no rows were skipped in this exercise due to invalid format ===

#the file is read using csv.DictREader, which gives each fow as a dictionary
#keyed by column names.   for row_num, row in enumerate(reader, start=2)
# because row on is the header.
# parses date date=datetime.strptime(row['date], '%Y-%-m-%d)
# if the amount can't be converted into a float the row is skilled
# a message is tored in skipped_rows with row number and the issue
# continue ensure that the current iteration is aborted so no transaction is added
# if no error occur, the tranaction is added to the transactions list

